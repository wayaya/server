...
public class DBUtil {
    ...
    public static void init(IConfig config) {
        ...
        if (comboPooledDataSource == null) {
            ...
            if (IsEmbedDB) {
                migrateLocation = System.getProperty("migrate", "filesystem:./migrate/h2");
//                migrateLocation = "filesystem:./migrate/h2";
                logger.debug("数据库迁移 schema 路径:{}", migrateLocation);
                logger.debug("初始化 内存h2 数据库连接池 配置文件位置:{}", System.getProperty("com.mchange.v2.c3p0.cfg.xml"));
    ...
    public static Connection getConnection() throws SQLException {
        ...
        if (map.get(threadId) != null) {
            logger.error("error here!!!! DB connection not close correctly");
        }
        map.put(threadId, Thread.currentThread().getStackTrace().toString());
        Connection connection = transactionConnection.get();
        if (connection != null) {
            logger.debug("Thread {} get db connection {}", threadId, connection);
            return connection;
        }
    ...
    public static void closeDB(Connection conn, PreparedStatement ps, ResultSet rs) {
        logger.debug("Thread {} release db connection {}", Thread.currentThread().getId(), conn);
        try {
            if (rs != null) {
                rs.close();
            }
